-- Drop and recreate user role functions for Jobsy AI
-- This script safely removes existing functions and creates new ones with correct signatures

-- Enable UUID extension if not already enabled
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";

-- Create user_roles table if it doesn't exist (with minimal structure)
...
-- Grant permissions (keeping existing RLS settings intact)
...
DROP FUNCTION IF EXISTS grant_user_role(UUID, TEXT, TEXT, UUID, INTEGER);
DROP FUNCTION IF EXISTS grant_user_role(UUID, TEXT, TEXT, UUID);
DROP FUNCTION IF EXISTS grant_user_role(TEXT, TEXT, UUID, INTEGER, TEXT);
DROP FUNCTION IF EXISTS grant_user_role(TEXT, TEXT, UUID, INTEGER);
DROP FUNCTION IF EXISTS grant_user_role(TEXT, TEXT, UUID);

DROP FUNCTION IF EXISTS revoke_user_role(UUID, UUID, TEXT);
DROP FUNCTION IF EXISTS revoke_user_role(UUID, UUID);
DROP FUNCTION IF EXISTS revoke_user_role(UUID, TEXT);

DROP FUNCTION IF EXISTS expire_user_roles();

-- Create the grant_user_role function (FIXED - all parameters have defaults)
...
BEGIN
    ...
    IF revoked_by_id IS NULL THEN
        RETURN jsonb_build_object(
            'success', false,
            'message', 'revoked_by_id is required'
        );
    END IF;
